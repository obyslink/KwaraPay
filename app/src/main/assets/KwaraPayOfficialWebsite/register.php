...
"http://www.w3.org/TR/xhtml1/DTD/ xhtml1-transitional.dtd"> 
	...
	require	( 'C:\\xampp\htdocs\kwarapay official website\mysqli_connect.php'); 
	...
	if	(preg_match	('/^\w{4,20}$/',	$trimmed['AccountNumber']))	{ 
	$AccountNumber	=	mysqli_real_escape_string($dbc,	$trimmed['AccountNumber']); 
	}	else	{ 
	echo	'<p	class="error">Please	enter	your	account number!</p>'; 
	} 
	//	Check	for	a	mobile number: 
	if	(preg_match	('/^\w{4,20}$/',	$trimmed['MobileNumber']))	{ 
	$MobileNumber	=	mysqli_real_escape_string($dbc,	$trimmed['AccountNumber']); 
	}	else	{ 
	...
	$q	=	"INSERT	INTO	users	(email,	pass,	first_name,	last_name, AccountNumber, MobileNumber,	active,	registration_ date)	VALUES	('$e',	SHA1('$p'),	'$fn',	'$ln',	'$a',	NOW(	)	)"; 
	$r	=	mysqli_query	($dbc,	$q)	or	trigger_error("Query:	$q\n<br	/>MySQL	Error:	"	.	mysqli_error($dbc)); 

	if	(mysqli_affected_rows($dbc)	==	1)	{	//	If	it	ran	OK. 

	//	Send	the	email: 
	$body	=	"Thank	you	for	registering	at Kwara-Pay Official Website.	To	activate	your	account,	please	click	on	this	link:\n\n"; 
	...
	<fieldset> 
	<p><b>First	Name:</b>	<input	type="text"	name="first_name"	size="20"	maxlength="20"	value="<?php	if	(isset($trimmed['first_name']))	
		echo	$trimmed['first_name'];	?>"	/></p> 
	<p><b>Last	Name:</b>	<input	type="text"	name="last_name"	size="20"	maxlength="40"	value="<?php	if	(isset($trimmed['last_name']))
	...
	<p><b>Password:</b>	<input	type="password"	name="password1"	size="20"	maxlength="20"	value="<?php	if	(isset($trimmed['password1']))	
		echo	$trimmed['password1'];	?>"	/>	<small>Use	only	letters,	numbers,	and	the	underscore.	Must	be	between	4	and	20	characters	long.	</small></p> 
